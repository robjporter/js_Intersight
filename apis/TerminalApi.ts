 ...
 * no description
 */
export class TerminalApiRequestFactory extends BaseAPIRequestFactory {
	
    /**
     * Read a 'terminal.AuditLog' resource.
     * @param moid The unique Moid identifier of a resource instance.
     */
    public async getTerminalAuditLogByMoid(moid: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
        // verify required parameter 'moid' is not null or undefined
        if (moid === null || moid === undefined) {
            throw new RequiredError('Required parameter moid was null or undefined when calling getTerminalAuditLogByMoid.');
        }

		
		// Path Params
    	const localVarPath = '/terminal/AuditLogs/{Moid}'
     ...
     */
    public async getTerminalAuditLogList(filter?: string, orderby?: string, top?: number, skip?: number, select?: string, expand?: string, apply?: string, count?: boolean, inlinecount?: 'allpages' | 'none', at?: string, tags?: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		...
		// Path Params
    	const localVarPath = '/terminal/AuditLogs';
...
export class TerminalApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getTerminalAuditLogByMoid
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTerminalAuditLogByMoid(response: ResponseContext): Promise<TerminalAuditLog > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TerminalAuditLog = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getTerminalAuditLogList
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTerminalAuditLogList(response: ResponseContext): Promise<TerminalAuditLogResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TerminalAuditLogResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "TerminalAuditLogResponse", ""
            ) as TerminalAuditLogResponse;
            return body;
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: Error = ObjectSerializer.deserialize(
