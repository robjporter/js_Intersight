 ...
 * no description
 */
export class TopApiRequestFactory extends BaseAPIRequestFactory {
	
    /**
     * Read a 'top.System' resource.
     * @param moid The unique Moid identifier of a resource instance.
     */
    public async getTopSystemByMoid(moid: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
        // verify required parameter 'moid' is not null or undefined
        if (moid === null || moid === undefined) {
            throw new RequiredError('Required parameter moid was null or undefined when calling getTopSystemByMoid.');
        }

		
		// Path Params
    	const localVarPath = '/top/Systems/{Moid}'
     ...
     */
    public async getTopSystemList(filter?: string, orderby?: string, top?: number, skip?: number, select?: string, expand?: string, apply?: string, count?: boolean, inlinecount?: 'allpages' | 'none', at?: string, tags?: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		...
		// Path Params
    	const localVarPath = '/top/Systems';
     ...
     */
    public async patchTopSystem(moid: string, topSystem: TopSystem, ifMatch?: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
        // verify required parameter 'moid' is not null or undefined
        if (moid === null || moid === undefined) {
            throw new RequiredError('Required parameter moid was null or undefined when calling patchTopSystem.');
        }

		
        // verify required parameter 'topSystem' is not null or undefined
        if (topSystem === null || topSystem === undefined) {
            throw new RequiredError('Required parameter topSystem was null or undefined when calling patchTopSystem.');
        }
        ...
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(topSystem, "TopSystem", ""),
            contentType
        );
     ...
     * @param ifMatch For methods that apply server-side changes, and in particular for PUT, If-Match can be used to prevent the lost update problem. It can check if the modification of a resource that the user wants to upload will not override another change that has been done since the original resource was fetched. If the request cannot be fulfilled, the 412 (Precondition Failed) response is returned. When modifying a resource using POST or PUT, the If-Match header must be set to the value of the resource ModTime property after which no lost update problem should occur. For example, a client send a GET request to obtain a resource, which includes the ModTime property. The ModTime indicates the last time the resource was created or modified. The client then sends a POST or PUT request with the If-Match header set to the ModTime property of the resource as obtained in the GET request.
     */
    public async updateTopSystem(moid: string, topSystem: TopSystem, ifMatch?: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
        // verify required parameter 'moid' is not null or undefined
        if (moid === null || moid === undefined) {
            throw new RequiredError('Required parameter moid was null or undefined when calling updateTopSystem.');
        }

		
        // verify required parameter 'topSystem' is not null or undefined
        if (topSystem === null || topSystem === undefined) {
            throw new RequiredError('Required parameter topSystem was null or undefined when calling updateTopSystem.');
        }
...
export class TopApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getTopSystemByMoid
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTopSystemByMoid(response: ResponseContext): Promise<TopSystem > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TopSystem = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getTopSystemList
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getTopSystemList(response: ResponseContext): Promise<TopSystemResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TopSystemResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "TopSystemResponse", ""
            ) as TopSystemResponse;
            return body;
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: Error = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to patchTopSystem
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async patchTopSystem(response: ResponseContext): Promise<TopSystem > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TopSystem = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to updateTopSystem
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async updateTopSystem(response: ResponseContext): Promise<TopSystem > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TopSystem = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
