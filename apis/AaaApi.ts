 ...
 * no description
 */
export class AaaApiRequestFactory extends BaseAPIRequestFactory {
	
    /**
     * Read a 'aaa.AuditRecord' resource.
     * @param moid The unique Moid identifier of a resource instance.
     */
    public async getAaaAuditRecordByMoid(moid: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
        // verify required parameter 'moid' is not null or undefined
        if (moid === null || moid === undefined) {
            throw new RequiredError('Required parameter moid was null or undefined when calling getAaaAuditRecordByMoid.');
        }

		
		// Path Params
    	const localVarPath = '/aaa/AuditRecords/{Moid}'
     ...
     */
    public async getAaaAuditRecordList(filter?: string, orderby?: string, top?: number, skip?: number, select?: string, expand?: string, apply?: string, count?: boolean, inlinecount?: 'allpages' | 'none', at?: string, tags?: string, options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		...
		// Path Params
    	const localVarPath = '/aaa/AuditRecords';
...
export class AaaApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getAaaAuditRecordByMoid
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getAaaAuditRecordByMoid(response: ResponseContext): Promise<AaaAuditRecord > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AaaAuditRecord = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getAaaAuditRecordList
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getAaaAuditRecordList(response: ResponseContext): Promise<AaaAuditRecordResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AaaAuditRecordResponse = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "AaaAuditRecordResponse", ""
            ) as AaaAuditRecordResponse;
            return body;
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: Error = ObjectSerializer.deserialize(
